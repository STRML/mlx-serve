...
/// leaves the fused MoE rows and GDN recurSeq paths. The gate compares ratios.
...
pub fn gate(m: ?Match, remaining: u32, lookup_ema: f32, mtp_ema: f32, mtp_width: u32, streak: bool) u32 {
    ...
    const lookup_rate = ((if (streak) kf else @min(lookup_ema, kf)) + 1) / verifyCost(k + 1);
    const mtp_cost = verifyCost(mtp_width + 1) + MTP_DRAFT_COST * @as(f32, @floatFromInt(mtp_width));
